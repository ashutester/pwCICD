...
if (!fs.existsSync(reportPath)) {
  console.log('⚠️  No test results found, creating empty metrics');
  const emptyMetrics = {
  ...
  // Add new metrics
  allMetrics.push(emptyMetrics);

  // Save
  fs.writeFileSync(dataFile, JSON.stringify(allMetrics, null, 2));
  console.log('✅ Empty metrics saved');
  process.exit(0);
}
...
try {
  ...
  console.log(`✅ Dashboard data saved (${allMetrics.length} records)`);
} catch (error) {
